...
func SJFSchedule(w io.Writer, title string, processes []Process) {
	...
	for i := 0; i < len(processes); i++ {
		...
		//fmt.Println("Current Process: ", processes[i].ProcessID)

		if processes[i].ArrivalTime > 0 {
			//waitTime = time process i entered CPU - time processes arrived
			waitingTime = serviceTime - processes[i].ArrivalTime

			//fmt.Println("\tWait Time Updated | New waitTime: ", waitingTime)
		}
		...
		//fmt.Println("\tService Time: ", serviceTime)
...
func sortByBurstDesc(processes []Process) {
	...
	for i := 0; i < len(processes); i++ {
		...
		/* 		fmt.Println("Arrival Time update: ")
		   		fmt.Println("\tProcess ID: ", processes[i].ProcessID)
		   		fmt.Println("\tArrival New Arrival Time: ", processes[i].ArrivalTime) */
